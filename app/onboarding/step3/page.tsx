...
export default function Step3() {
  ...
  const handleRemoveSkill = (skillToRemove: string) => {
    const currentSkills = form.getValues('expertise');
    form.setValue('expertise', currentSkills.filter(skill => skill !== skillToRemove));
  };

  // Gestionnaire pour supprimer un intérêt
  const handleRemoveInterest = (interestToRemove: string) => {
    const currentInterests = form.getValues('expertise');
    form.setValue('expertise', currentInterests.filter(interest => interest !== interestToRemove));
  };
